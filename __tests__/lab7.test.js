describe('Basic user flow for Website', () => {
  ...
  it('Checking number of items in cart on screen', async () => {
     ...
     * Remember to remove the .skip from this it once you are finished writing this test.
     */
    // Asegurarse de agregar al carrito sólo los que aún no están agregados
    ...
    // checking if the count is the same as the one in the page
    const count = await page.$eval('#cart-count', el => el.innerText);
    expect(count).toBe('20');
    // had to modify the time here because it was timing out.
    // I also did the same to the other tests just in case
  }, 30000);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    // I think I explained myself here
    const count = await page.$eval('#cart-count', el => el.innerText);
    expect(count).toBe('0');
  }, 30000);
  ...
  it('Checking the localStorage to make sure cart is correct', async () => {
     ...
     */
    //WOOOOOO DONEEE
    console.log('Checking the localStorage for empty cart...');
    const cart = await page.evaluate(() => localStorage.getItem('cart'));
    expect(cart).toBe('[]');
  });
});